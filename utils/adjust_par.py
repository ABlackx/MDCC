...
def adjust_consistency_weight(iter, max_iters, weight_min=0.1, weight_max=1.0, warmup_iters=0):
    """Adjusts the consistency loss weight with a cosine schedule"""
    if iter < warmup_iters:
        weight_decay = iter / warmup_iters
        return weight_min + (weight_max - weight_min) * weight_decay
    else:
        weight_decay = 0.5 * (1. + math.cos(math.pi + math.pi * (iter - warmup_iters) / (max_iters - warmup_iters)))
        return weight_min + (weight_max - weight_min) * weight_decay
    # if iter < warmup_iters:
    #     # Linearly increase during warmup period
    #     weight = weight_min + (weight_max - weight_min) * (iter / warmup_iters)
    # else:
    #     # After warmup, keep weight steady
    #     weight = weight_max
    # return weight
